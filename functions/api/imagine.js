...
// ────────────────────────────────────────────────────────────────────
// Vertex AI Imagen 3 — 서비스 계정 JWT 인증 + inpainting edit
// ────────────────────────────────────────────────────────────────────
...
async function getVertexAccessToken(env) {
  ...
  if (!tokenRes.ok) throw new Error(`Vertex token error: ${await tokenRes.text()}`);
  const { access_token } = await tokenRes.json();
  return access_token;
}

async function callVertexInpaint(env, imageBase64, prompt) {
  const accessToken = await getVertexAccessToken(env);
  const url = `https://us-central1-aiplatform.googleapis.com/v1/projects/allososales-20260306/locations/us-central1/publishers/google/models/imagen-3.0-capability-001:predict`;
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      instances: [{
        ...
        mask: { maskMode: { maskType: 'FOREGROUND' } },
      }],
      parameters: {
  ...
  if (!res.ok) throw new Error(`Vertex Imagen ${res.status}: ${(await res.text()).slice(0, 300)}`);
...
async function callGemini(env, imageBase64s, prompt) {
  ...
  //   gemini-2.0-flash-preview-image-generation — 미리보기 (개선됨)
  //   gemini-2.0-flash-preview-image-generation-image-generation — 실험판 (다른 특성)
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (mode === 'swap_material') {
      ...
      // ── 위치/배치 키워드 감지 → place 스타일 vs swap 스타일 자동 분기 ──
      // 위치 명시가 있으면 "이 거실의 X 위치에 배치" (제거 없음)
      // 위치 명시가 없으면 기존 동작 (가구 교체)
      ...
      for (const [type, words] of Object.entries(placementKeywords)) {
        for (const w of words) {
          if (instruction.includes(w)) {
            matchedPlacements.push({ type, word: w });
            break; // 같은 타입은 한 번만
          }
        }
      }
      // 방향 보조 키워드 ("등판이 X벽에 붙은", "X쪽으로 향해" 등)
      ...
      // place 모드 진입 조건: 위치 키워드 또는 방향 힌트가 명시되면 무조건 place
      // 추가로 "배치", "넣어", "놓아", "두어" 동사 + 시리즈명 조합도 place
      ...
      if (targetSeries && targetSeriesFolder && env.PRODUCTS) {
        ...
        if (usePlaceMode) {
          // ── PLACE MODE: 거실 사진 + 가구 누끼 → 가구를 거실에 추가 배치 (제거 없음) ──
          ...
          // 짧고 구체적인 프롬프트 (Gemini 웹앱에서 잘 동작했던 스타일)
          const colorClauseForPlace = matchedColors.length > 0
            ? `Color: ${matchedColors.join(', ')}.`
            : '';
          seriesSwapPrompt = [
        ...
        return json({
          ...
          placement: matchedPlacements.map(p => p.word),
          orientation: orientationHints,
      ...
      // ── 색상 설명 텍스트 조합 (Vertex/Gemini 공용) ──
      const colorDesc = chipMeta
        ? `${chipMeta.colorKo} (${enrichment.trim() || instruction})`
        : instruction + (enrichment ? ` ${enrichment}` : '');

      // ── Vertex AI Imagen 3 inpainting 프롬프트 (간결하게) ──
      const vertexPrompt = `Recolor only the sofa/furniture upholstery surface to: ${colorDesc}. ` +
        `Keep the EXACT same sofa position, shape, cushions, room, background, lighting, and camera angle. ` +
        `Do NOT move or resize the sofa. Only the fabric/leather surface color changes.`;

      // ── Gemini fallback용 프롬프트 (기존 유지) ──
      ...
      const swapPrompt = [
        ...
        `4. The EXACT camera angle, perspective, framing, crop, focal length — and the EXACT pixel position of the furniture within the frame. Do NOT shift, move, recompose, or offset the sofa or any object left, right, up, or down even by a single pixel.`,
        ...
        chipBase64
          ? `Match IMAGE 2's color and material texture EXACTLY. Sample the swatch carefully — match its hue, saturation, value, warmth, and surface texture precisely. Do not approximate. CRITICAL: Do NOT darken, intensify, or oversaturate the swatch color.`
      ...
      // Vertex AI 우선 시도 → 실패 시 Gemini fallback
      let vertexError = null;
      if (env.GOOGLE_SERVICE_ACCOUNT_JSON) {
        try {
          resultBase64 = await callVertexInpaint(env, base64, vertexPrompt);
          if (resultBase64) provider = 'vertex_imagen3';
        } catch (e) {
          vertexError = e.message;
          console.error('Vertex AI failed, falling back to Gemini:', e.message);
        }
      } else {
        vertexError = 'GOOGLE_SERVICE_ACCOUNT_JSON not set';
      }
      ...
      return json({
        ...
        chip: chipMeta,
        chip_attempted_key: chipAttemptedKey,
        chip_fetched: !!chipBase64,
        chip_error: chipFetchError,
